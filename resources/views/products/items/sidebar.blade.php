    ...
    @foreach($filters as $categoryName => $subcats)
        <div class="category-group mt-4">
            ...
            <div class="category-subcategories ml-4 mt-2 hidden">
                @foreach($subcats as $charName => $values)
                    @foreach($values as $id => $display)
                        <label class="flex items-center py-1 filter-option">
                            <input type="checkbox" class="mr-2 subcategory-filter" data-subcategory="{{ $charName }}" data-value="{{ $id }}">
                            <span class="text-sm custom-text-secondary">{{ $display }}</span>
                        </label>
                    @endforeach
                @endforeach
            </div>
